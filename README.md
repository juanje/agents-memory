...
- [Import history (optional bootstrap)](#import-history-optional-bootstrap)
...
4. Symlink commands globally:
   - Cursor:
     ...
     ln -s ~/agents-memory/commands/import-history.md ~/.cursor/commands/import-history.md
   - claude-code:
     ...
     ln -s ~/agents-memory/commands/import-history.md ~/.claude/commands/import-history.md
...
1. Clone this repo:
  ```bash
   git clone https://github.com/juanje/agents-memory ~/agents-memory
  ```
2. Configure hooks — sessionStart for context injection, plus auto-save hooks for afterAgentResponse, preCompact, and sessionEnd (see setup prompt above for the exact JSON for both Cursor and claude-code).
3. Symlink commands:
  ```bash
   ln -s ~/agents-memory/commands/save.md ~/.cursor/commands/save.md
   ln -s ~/agents-memory/commands/consolidate.md ~/.cursor/commands/consolidate.md
...
A project's `index.md` declares its stack and team. The sessionStart hook composes context from all relevant scopes automatically.

## Import history (optional bootstrap)

Already have weeks of coding sessions in Cursor or claude-code? Import them to bootstrap your memory instead of starting from zero.

**Option A: Use the command (recommended)** — let the agent handle it:

```bash
# Add the command symlink (one-time, during setup):
ln -s ~/agents-memory/commands/import-history.md ~/.cursor/commands/import-history.md
ln -s ~/agents-memory/commands/import-history.md ~/.claude/commands/import-history.md
```

Then tell your agent in natural language:

```
/import-history from the last two weeks for my-api and my-cli
```

The agent reads the command docs, runs the script with the right flags, and reports progress.

**Option B: Run the script directly:**

```bash
# See what's available (last 30 days by default)
...
python3 ~/agents-memory/scripts/import-history.py --project my-api my-cli

# Import last 7 days (quick start)
...
| `--since N`            | Days of history to import (default: 30) |
| `--all`                | No date limit                           |
| `--project NAME [...]` | Only import specific projects           |
| `--limit N`            | Process at most N transcripts           |
| `--min-lines N`        | Skip short transcripts (default: 10)    |
| `--cli agent           | claude`                                 |


Projects with a `.skip` file are excluded automatically. The import is idempotent — already-imported sessions are skipped on re-run.

> **Time estimate:** Each transcript takes ~30–90 seconds to process
> (extraction via headless agent). A typical 30-day import with 5–10
> active projects takes 30–90 minutes. Full history imports can take
> several hours. The process runs unattended — start it and come back.
...
[https://github.com/juanje/](https://github.com/juanje/)